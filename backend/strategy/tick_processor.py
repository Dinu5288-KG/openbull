...
async def _process_tick_for_run(
    ...
    async with state_module.get_state_lock(run_id):
        ...
        for lid, leg in matched_legs:
            ...
            target_mode = _risk_mode(leg_config, "target_mode", "TGT: pts")
            use_underlying_risk = (
                is_underlying_match
                and (_is_underlying_mode(sl_mode) or _is_underlying_mode(target_mode))
            )
            ...
            if use_underlying_risk:
                ...
                sl_level, target_level = _underlying_risk_levels(
                ...
                if target_level is not None:
                    leg["effective_target"] = target_level
                state_changed = True

                triggered = _underlying_risk_trigger(
                    leg_config=leg_config,
                    position=leg.get("position"),
                    underlying_ltp=ltp,
                    sl_level=sl_level,
                ...
                if triggered:
                    outcome = risk_evaluator.RiskOutcome(
                        ...
                        triggered=triggered,
                        sl_at=sl_level if triggered == "sl" else None,
                        target_at=target_level if triggered == "target" else None,
                    )
            ...
            outcome = risk_evaluator.evaluate_leg(
                ...
                target_pts=None if _is_underlying_mode(target_mode) else leg_config.get("target_pts"),
    ...
    *,
    ...
    if _is_underlying_mode(sl_mode):
        ...
        if sl_delta is not None:
            sl_level = underlying_entry - exposure_sign * sl_delta

    target_level = None
    if _is_underlying_mode(target_mode):
        target_delta = _risk_delta(
            underlying_entry, leg_config.get("target_pts"), target_mode,
        )
        if target_delta is not None:
            target_level = underlying_entry + exposure_sign * target_delta

    return sl_level, target_level


def _underlying_risk_trigger(
    *,
    leg_config: dict[str, Any],
    position: str,
    ...
    if sl_level is not None:
        ...
        if exposure_sign < 0 and underlying_ltp >= sl_level:
            return "sl"
    if target_level is not None:
        if exposure_sign > 0 and underlying_ltp >= target_level:
            return "target"
        if exposure_sign < 0 and underlying_ltp <= target_level:
            return "target"
    return None
